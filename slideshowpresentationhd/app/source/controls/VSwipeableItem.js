...
enyo.kind({
	...
	chrome: [
		{name: "confirm", canGenerate: false, showing: false, kind: "CScrimmedConfirmPrompt", className: "enyo-fit", onConfirm: "confirmSwipe", onCancel: "cancelSwipe"}
	],
	statified: {
	...
	dragstartHandler: function(inSender, inEvent) {
		this.resetPosition();
		if (this.swipeable && inEvent.vertical && !this.confirmShowing && this.hasNode()) {
	...
	dragHandler: function(inSender, inEvent) {
		...
		if (this.handlingDrag) {
			if (this.hasNode()) {
				this.node.style.webkitTransform = "translate3d(0, " + dx + "px, 0)";
				this.doDrag(dx);
			} else {
	...
	getDx: function(inEvent) {
		...
		return inEvent.dy > 0 || this.allowLeft ? inEvent.dy : 0;
	},
	fetchTriggerDistance: function() {
		var w = this.getBounds().height || 0;
...
enyo.kind({
	...
	chrome: [
		...
		{name: "confirm", kind: "CConfirmPrompt", className: "enyo-fit", domStyles: {"z-index": 2}, onConfirm: "doConfirm", onCancel: "doCancel"}
	],
	//* @protected
	create: function() {
		this.inherited(arguments);
